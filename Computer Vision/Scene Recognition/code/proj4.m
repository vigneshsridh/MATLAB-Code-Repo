...
abbr_categories = {'Kit', 'Sto', 'Bed', 'Liv', 'Off', 'Ind', 'Sub', 'Cty', 'Bld', 'St', 'HW', 'OC', 'Cst', 'Mnt', 'For'};
...
switch lower(FEATURE)    
    ...
    case 'bag of sift'
        ...
        if ~exist('vocab400.mat', 'file')
            ...
            save('vocab400.mat', 'vocab')
        else
            load('vocab400.mat');
        end
        
        % YOU CODE get_bags_of_sifts.m
        if ~exist('train_get_bags_400.mat', 'file')
            fprintf('No existing bag of features found. Computing one from training images\n')
            %vocab_size = 400; %Larger values will work better (to a point) but be slower to compute
            train_image_feats = get_bags_of_sifts(train_image_paths);
            save('train_get_bags_400.mat', 'train_image_feats')
        else
            load('train_get_bags_400.mat');
        end
        
         if ~exist('test_get_bags_400.mat', 'file')
            fprintf('No existing bag of features found. Computing one from testing images\n')
            %vocab_size = 400; %Larger values will work better (to a point) but be slower to compute
            test_image_feats = get_bags_of_sifts(test_image_paths);
            save('test_get_bags_400.mat', 'test_image_feats')
         else
             load('test_get_bags_400.mat');
         end
        
% To add GIST             
%             train_image_feats  = gist_added(train_image_paths, vocab);
%             test_image_feats  = gist_added(test_image_paths, vocab);
          
            
% To add GIST, Gaussian Pyramid functionality            
%             train_image_feats  = gaussian_pyramid(train_image_paths, vocab);
%             test_image_feats  = gaussian_pyramid(test_image_paths, vocab);

% To add GIST, Spatial Pyramid functionality 
